document.addEventListener('DOMContentLoaded', () => {
    ...
    // RECEPCIÓN Y AUTO-DESPLIEGUE DE MENSAJES
    socket.on('receiveMessage', (msg) => {
        // 1. Si es un mensaje privado y aún no existe la tarjeta en el menú lateral, la creamos
        if (msg.chatId !== 'global-chat') {
            ...
            // Actualizar la vista previa del último mensaje en la tarjeta lateral
        ...
        // 2. Si ya tienes esa conversación abierta en la pantalla central:
        if (msg.chatId === currentChatId) {
            renderMessage(msg);
            if (msg.senderName !== currentUser.name) playNotificationSound();
        } else {
            // 3. SI TE ESCRIBEN DESDE OTRO CHAT: Abre la conversación automáticamente para que veas el mensaje
            if (msg.senderName !== currentUser.name) {
                ...
                if (card) {
                    ...
                    const otherUser = msg.senderName;
                    const chatAvatar = card.dataset.chatAvatar || `https://api.dicebear.com/7.x/bottts/svg?seed=${otherUser}`;
                    
                    // Cambia la vista activa al nuevo chat y carga su historial
                    selectChat(msg.chatId, otherUser, chatAvatar);
                }
            }
        }
    });
