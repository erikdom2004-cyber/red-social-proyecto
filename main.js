document.addEventListener('DOMContentLoaded', () => {
    ...
    // GENERADOR UNIFORME DE ID PRIVADO
    ...
    function selectChat(chatId, chatName, chatAvatar) {
        ...
        // Limpiar la pantalla de mensajes viejos inmediatamente al cambiar de chat
        const messagesContainer = document.getElementById('messages-container');
        if (messagesContainer) {
            messagesContainer.innerHTML = '<div class="date-divider"><span>Cargando mensajes...</span></div>';
        }

        // Quitar indicador de no leído de la tarjeta
    ...
    socket.on('receiveMessage', (msg) => {
        // 1. Si es mensaje privado y no tenemos la tarjeta creada en el sidebar, la generamos
        if (msg.chatId !== 'global-chat') {
            ...
            // Actualizar vista previa del texto en la tarjeta lateral
            if (privateChatCard) {
                const lastMsgElem = privateChatCard.querySelector('.last-message');
                if (lastMsgElem) {
                    lastMsgElem.textContent = `${msg.senderName}: ${msg.type === 'text' ? msg.content : '[' + msg.type.toUpperCase() + ']'}`;
                }
            }
        }

        // 2. Si es para el chat que tengo ABIERTO en pantalla:
        if (msg.chatId === currentChatId) {
            renderMessage(msg);
            if (msg.senderName !== currentUser.name) playNotificationSound();
        } else {
            // 3. Si es para otro chat en segundo plano:
            if (msg.senderName !== currentUser.name) {
                playNotificationSound();

                // Agregar indicador de "Nuevo" en la tarjeta
